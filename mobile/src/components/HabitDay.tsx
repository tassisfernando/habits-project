...
export const DAY_SIZE = Dimensions.get('screen').width / WEEEK_DAYS - (SCREEN_HORIZONTAL_PADDING + 5);
...
export function HabitDay({ amountOfHabits = 0, amountCompleted = 0, date, ...rest }: Props) {
  const amountAccomplishedPercentage = amountOfHabits > 0 ?
    generateProgressPercentage(amountOfHabits, amountCompleted) : 0;
  ...
  return (
    <TouchableOpacity
      className={clsx("rounded-lg border-2 m-1", {
        ...
        ["border-green-400 border-4"]: isCurrentDay
      })}
